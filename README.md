...
| `legacyjournal_actions.lua` | 原版风格 TimedAction、动画和整页检查点 |
| `legacyjournal_client.lua` | 右键菜单、物品转移和多人请求 |
| `legacyjournal_server.lua` | 服务端验证、事务令牌、提交和同步 |
| `legacyjournal_presentation.lua` | 本地化名称与显示刷新 |
| `legacyjournal_skillbook_compat.lua` | Build 42 技能书兼容 |

多人数据流：客户端发送 `begin` 意图，服务器重新定位物品并计算差值，随后签发一次性令牌；
TimedAction 只提交进度和最终 `commit`，服务器再次验证签名后才原子写入或恢复。单人模式复用
相同的共享规则，不维护第二套玩法实现。
...
- 服务端权威保护知识内容与事务重放，但现实耗时目前不是强防作弊边界。
- 极端网络故障导致 `begin` 回包丢失时，客户端可能需要重新连接。
...
| `legacyjournal_actions.lua` | Vanilla-style TimedActions, animation, and page checkpoints |
| `legacyjournal_client.lua` | Context menus, item transfer, and multiplayer requests |
| `legacyjournal_server.lua` | Server validation, transaction tokens, commits, and synchronization |
...
In multiplayer, the client sends a `begin` intent, the server relocates the item and computes the delta,
and then issues a one-use token. The TimedAction submits only progress and the final `commit`; the server
revalidates the signature before atomically writing or restoring state. Single-player reuses the same
shared rules rather than maintaining a second gameplay implementation.
...
- Server authority protects knowledge state and transaction replay, but elapsed real time is not a hard
  anti-cheat boundary.
- A lost `begin` response in an extreme network failure may require reconnecting.
